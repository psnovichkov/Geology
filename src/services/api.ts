...
export class API {
    ...
    public static readonly API_URL = 'https://x9hjtrdwd3.execute-api.us-east-1.amazonaws.com/prod';
    ...
    private static fetchData<T,S>(path: string, method: string, params: T ): Promise<S> {
        return fetch(API.API_URL + path, {
            method: method,
            headers: {
                'Content-Type': 'application/json'
    ...
    public static searchByFilter(params: SearchFilterParams): Promise<Sample[]> {
        return this.fetchData<SearchFilterParams, Sample[]>('/search', 'POST', params);
    }
    ...
    public static searchByLocation(params: SearchLocationParams): Promise<Sample[]> {
        return this.fetchData<SearchLocationParams, Sample[]>('/search', 'POST', params);
    }
    ...
    public static login(username: string, password: string): Promise<string> {
        return this.fetchData<{ username: string, password: string }, string>('/login', 'POST', { username, password });
    }
}
